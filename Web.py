...
model2=joblib.load("Language_detection.pkl")
model3=joblib.load("semantic_Analysis.pkl")


def detect_lang(language):
    prediction =model2.predict([language])
    return prediction[0]


def predict_trip_price(source, destination, cab_type, name, surge_multiplier,distance):
    prediction =model1.predict( pd.DataFrame({"source":[source],
...
def evaluate_review(review):
    prediction =model3.predict(review )
    if prediction[0]==1:
...
def text_preprocessing(text):           # language Detection
...
def preprocessing (text):               # Semantic Analysis
...
def main():
    ...
    if page=="Detect":
       ...
       html_temp = """
                        <div style="background-color:tomato;padding:10px">
                        <h2 style="color:white;text-align:center;">Identify language of text</h2>
                        </div>
                   """
       st.markdown(html_temp,unsafe_allow_html=True)
       st.title("")
       lang =st.text_input("Please Enter text here.")
       lang=text_preprocessing(lang)
       result=detect_lang(lang)
       if st.button("Detect"):
          st.success("Language : {} ".format(result))  
    ...
    if page=="Predict":
       ...
       if st.button("Predict"):
          ...
          st.success("Expected trip price : {} $".format(round(result))) 
            
    if page=="Evaluate":
       html_temp = """
                        <div style="background-color:tomato;padding:10px">
                        <h2 style="color:white;text-align:center;">Evaluate The Trip in Arabic</h2>
                        </div>
                   """
       st.markdown(html_temp,unsafe_allow_html=True)
       st.title("")

       if st.checkbox("Cascade",value=True):
          review =st.text_input("Please Enter Your Review in Arabic")
    
          if st.button("Predict"):
          
             if detect_lang(review)=="Arabic":
                review = preprocessing (review)
                res=evaluate_review([review])
                st.success("Evaluation : {} ".format(res)) 
             else: 
                lang=detect_lang(review)
                st.error("Oops, you entered an evaluation in {} ,and you must enter the evaluation in Arabic only. ".format(lang))
       else:
          review =st.text_input("Please Enter Your Review in Arabic")
          if st.button("Predict"):
             review = preprocessing (review)
